...
def ouroboros(i_target, i_in, wavelength, focal_length, dx, iter=10, iterations=100, visualize=False):
    ...
    mse_history = []
    for i in range(iter):
        phase_gs, mse, e_in = gerchberg_saxton_modified(i_target, i_in, wavelength, focal_length, dx, phase, e_in, ax1, ax2, ax3, mse_history, iterations, visualize)
        phase = phase_gs

    if visualize:
        plt.ioff()

    return phase_gs, mse_history, e_in

def gerchberg_saxton_modified(i_target, i_in, wavelength, focal_length, dx, phase, e_in, ax1, ax2, ax3, mse_history, iterations=100, visualize=False, mse_threshold=0.01e-9):
    ...
    for i in range(iterations):
        ...
        if i > 3 and abs(mse_history[-1] - mse_history[-2]) < mse_threshold:
...
def export_phase_as_stl(phase, x, y, wavelength, n=1.54, filename='phase_profile.stl'):
    """
    ...
    Для обеспечения положительных значений, к высотам добавляется оффсет.
    """
...
# Целевая интенсивность (крест толщиной 1 мм)
thickness = 2e-3  # 1 мм
pixels = int(thickness / dx)
i_target = np.zeros((resolution, resolution))
c = resolution // 2

reticle_length = 18e-3  # Общая длина "рукавов" прицела [м]
arm_pixels = int(reticle_length / dx)  # Длина каждого рукава в пикселях
# Горизонтальный бар (ограниченный по x)
#i_target[c - pixels : c + pixels, c - arm_pixels // 2 : c + arm_pixels // 2] = 1
# Вертикальный бар (ограниченный по y)
#i_target[c - arm_pixels // 2 : c + arm_pixels // 2, c - pixels : c + pixels] = 1
...
# Запуск алгоритма
phase_gs, mse, e_in = ouroboros(i_target, i_in, lambda_, f, dx, iter=1000, iterations=10, visualize=True)
...
plt.subplot(223)
plt.imshow(np.abs(angular_spectrum_propagation(e_in, lambda_, dx, dx, f)),cmap='gray')
...
print("Фазовый профиль успешно экспортирован в файл 'phase_profile.npy'.")
export_phase_as_stl(phase_gs, x, y, wavelength=lambda_, n=1.65, filename='phase_profile.stl')
